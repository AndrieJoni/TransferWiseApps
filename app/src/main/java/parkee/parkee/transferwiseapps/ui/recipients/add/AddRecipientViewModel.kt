...
import parkee.parkee.transferwiseapps.data.network.recipient.ValidationRequirementsDto
...
class AddRecipientViewModel(
    ...
    var goBackWithResultEvent = SingleLiveEvent<Any>()

    var currencyErrorEvent = SingleLiveEvent<Boolean>()
    var firstNameErrorEvent = SingleLiveEvent<Boolean>()
    ...
    fun currencySelected(data: CurrencyModel) {
        currentCurrencySelected = data
        getFormRequirements(currentCurrencySelected?.currencyName.toString())
    }
    ...
    fun handleCurrencyTextWatcher(currency: String) {
        currencyErrorEvent.value = currency.isEmpty()
    }

    fun handleFirstNameTextWatcher(firstName: String) {
        firstNameErrorEvent.value = firstName.isEmpty()
    }

    fun handleLastNameTextWather(lastName: String) {
        lastNameErrorEvent.value = lastName.isEmpty()
    }
    ...
    fun addRecipient(
        ...
        when {
            ...
            currentCurrencySelected == null -> {
                currencyErrorEvent.value = true
            }

            firstName.isEmpty() -> {
                firstNameErrorEvent.value = true
            }

            lastName.isEmpty() -> {
                lastNameErrorEvent.value = true
            }
